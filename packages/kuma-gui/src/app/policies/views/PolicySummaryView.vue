                  ...
                  >
                    ...
                    <div>
                      <XSelect
                        ...
                        :selected="item.spec || ['universal', 'k8s'].includes(route.params.format) ? route.params.format : 'universal'"
                        @change="(value) => {
                          route.update({ format: value })
                        }"
                        @vue:before-mount="!item.spec && $event?.props?.selected && ['universal', 'k8s'].includes($event.props.selected) && $event.props.selected !== route.params.format && route.update({ format: $event.props.selected })"
                      >
                        <template
                          v-for="value in [...(item.spec ? ['structured'] : []), 'universal', 'k8s']"
                          :key="value"
                          #[`${value}-option`]
                        >
