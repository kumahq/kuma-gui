<template>
  <RouteView
    name="zone-ingress-detail-view"
    :params="{
      subscription: '',
      proxy: '',
      inactive: Boolean,
    }"
    v-slot="{ t, route, uri }"
  >
    <AppView>
      <XCard
        data-testid="zone-ingress-about-section"
      >
        <template #title>
          {{ t('zone-ingresses.routes.item.about.title') }}
        </template>
        <DataLoader
          :data="[props.data]"
          v-slot="{ data: [zoneIngress] }"
        >
          <XTimespan
            :start="zoneIngress.creationTime"
            :end="zoneIngress.modificationTime"
          />
          <XLayout variant="y-stack">
            <XDl variant="x-stack">
              <div>
                <dt>
                  {{ t('http.api.property.status') }}
                </dt>
                <dd>
                  <StatusBadge
                    :status="zoneIngress.state"
                  />
                </dd>
              </div>
              <div v-if="zoneIngress.namespace.length > 0">
                <dt>
                  {{ t('http.api.property.namespace') }}
                </dt>
                <dd>
                  <XBadge appearance="decorative">
                    {{ zoneIngress.namespace }}
                  </XBadge>
                </dd>
              </div>
              <div>
                <dt>
                  {{ t('http.api.property.address') }}
                </dt>
                <dd>
                  <template
                    v-if="zoneIngress.zoneIngress.socketAddress.length > 0"
                  >
                    <XCopyButton
                      variant="badge"
                      format="default"
                      :text="zoneIngress.zoneIngress.socketAddress"
                    />
                  </template>

                  <template v-else>
                    {{ t('common.detail.none') }}
                  </template>
                </dd>
              </div>
              <div>
                <dt>
                  {{ t('http.api.property.advertisedAddress') }}
                </dt>
                <dd>
                  <template
                    v-if="zoneIngress.zoneIngress.advertisedSocketAddress.length > 0"
                  >
                    <XBadge appearance="decorative">
                      <XCopyButton
                        :text="zoneIngress.zoneIngress.advertisedSocketAddress"
                      />
                    </XBadge>
                  </template>

                  <template v-else>
                    {{ t('common.detail.none') }}
                  </template>
                </dd>
              </div>

              <template
                v-for="labels in [Object.entries(zoneIngress.labels)]"
                :key="typeof labels"
              >
                <div v-if="labels.length > 0">
                  <dt>{{ t('services.routes.item.labels') }}</dt>
                  <dd>
                    <XLayout
                      variant="separated"
                      truncate
                    >
                      <template
                        v-for="kumaRe in [/^(.+\.)?kuma\.io\//]"
                        :key="typeof kumaRe"
                      >
                        <XBadge
                          v-for="[key, value] in labels"
                          :key="key"
                          :appearance="kumaRe.test(key) ? 'info' : 'decorative'"
                        >
                          {{ key }}:{{ value }}
                        </XBadge>
                      </template>
                    </XLayout>
                  </dd>
                </div>
              </template>
            </XDl>


            <XLayout
              v-if="zoneIngress.zoneIngressInsight.subscriptions.length > 0"
              variant="y-stack"
              data-testid="about-zone-ingress-subscriptions"
              class="about-subsection"
            >
              <XLayout variant="separated">
                <h3>{{ t('zone-ingresses.routes.item.subscriptions.title') }}</h3>
                <XAction
                  data-action
                  appearance="anchor"
                  :to="{
                    name: 'zone-ingress-subscriptions-list-view',
                    params: {
                      proxy: zoneIngress.kri,
                    },
                    query: {
                      inactive: route.params.inactive,
                    },
                  }"
                >
                  ({{ t('zone-ingresses.routes.item.subscriptions.show-details') }})
                </XAction>
              </XLayout>
              <template
                v-for="subscription in [zoneIngress.zoneIngressInsight.connectedSubscription]"
                :key="typeof subscription"
              >
                <template v-if="!subscription?.disconnectTime && subscription?.connectTime">
                  <XDl variant="x-stack">
                    <div>
                      <dt>
                        {{ t('zone-ingresses.routes.item.subscriptions.connected') }}
                      </dt>
                      <dd>
                        <XBadge appearance="neutral">
                          {{ t('common.formats.datetime', { value: Date.parse(subscription.connectTime) }) }}
                        </XBadge>
                      </dd>
                    </div>
                    <div>
                      <dt>
                        {{ t('zone-ingresses.routes.item.subscriptions.instance') }}
                      </dt>
                      <dd>
                        <XBadge appearance="info">
                          {{ subscription.controlPlaneInstanceId }}
                        </XBadge>
                      </dd>
                    </div>
                    <div>
                      <dt>
                        {{ t('zone-ingresses.routes.item.subscriptions.version') }}
                      </dt>
                      <dd>
                        <XBadge appearance="info">
                          {{ subscription.version?.kumaDp?.version ?? t('common.unknown') }}
                        </XBadge>
                      </dd>
                    </div>
                  </XDl>
                </template>
                <template v-else>
                  <XI18n path="zone-ingresses.routes.item.subscriptions.disconnected" />
                </template>
              </template>
            </XLayout>
          </XLayout>
        </DataLoader>
      </XCard>
      <XCard>
        <DataLoader
          :data="[props.data]"
        >
          <template #connecting>
            <XLayout
              variant="columns"
            >
              <XProgress />
              <XProgress />
            </XLayout>
          </template>
          <template #default="{ data: [zoneIngress] }">
            <DataLoader
              :src="uri(sources, '/connections/stats/for/:proxyType/:name/:mesh/:socketAddress', {
                name: zoneIngress.id,
                mesh: '*',
                socketAddress: zoneIngress.zoneIngress.socketAddress,
                proxyType: 'zone-ingress',
              })"
            >
              <template #connecting>
                <XLayout
                  variant="columns"
                >
                  <XProgress />
                  <XProgress />
                </XLayout>
              </template>
              <template #default="{ data: [traffic], refresh }">
                <XLayout
                  variant="columns"
                >
                  <ConnectionTraffic>
                    <XLayout
                      variant="y-stack"
                      size="small"
                    >
                      <DataCollection
                        type="inbounds"
                        :items="Object.entries(traffic.inbounds)"
                        v-slot="{ items }"
                      >
                        <template
                          v-for="[name, stats] in items"
                          :key="`${name}`"
                        >
                          <ConnectionCard
                            protocol=""
                            :traffic="stats"
                          >
                            <XAction
                              data-action
                              :to="{
                                name: ((name) => name.includes('bound') ? name.replace('-outbound-', '-inbound-') : 'zone-ingress-connection-inbound-summary-stats-view')(String(_route.name)),
                                params: {
                                  connection: name,
                                  proxyType: 'ingresses',
                                },
                                query: {
                                  inactive: route.params.inactive,
                                },
                              }"
                            >
                              :{{ name.split('_').at(-1) }}
                            </XAction>
                          </ConnectionCard>
                        </template>
                      </DataCollection>
                    </XLayout>
                  </ConnectionTraffic>
                  <ConnectionTraffic>
                    <template
                      #actions
                    >
                      <XInputSwitch
                        :checked="route.params.inactive"
                        data-testid="dataplane-outbounds-inactive-toggle"
                        @change="(value) => route.update({ inactive: value})"
                      >
                        <template #label>
                          Show inactive
                        </template>
                      </XInputSwitch>

                      <XAction
                        action="refresh"
                        appearance="primary"
                        @click="refresh"
                      >
                        Refresh
                      </XAction>
                    </template>

                    <!-- Outbounds for gateways report actual traffic on the upstream so we switch to upstream here for non-standard-->
                    <template
                      v-for="direction in ['upstream'] as const"
                      :key="direction"
                    >
                      <DataCollection
                        type="outbounds"
                        :items="Object.entries<any>(traffic.outbounds)"
                        v-slot="{ items: outboundEntries }"
                      >
                        <DataCollection
                          type="activeOutbounds"
                          :predicate="route.params.inactive ? undefined : ([_key, item]) => ((typeof item.tcp !== 'undefined' ? item.tcp?.[`${direction}_cx_rx_bytes_total`] : item.http?.[`${direction}_rq_total`]) as (number | undefined) ?? 0) > 0"
                          :items="outboundEntries"
                          v-slot="{ items: outbounds }"
                        >
                          <ConnectionGroup
                            v-if="outbounds.length > 0"
                            type="outbound"
                          >
                            <!-- the outbound name is the service name potentially with a 16 digit hash appended -->
                            <!-- that potential hash follows a hypen and can contain digits and a-f -->
                            <!-- so we replace this with `` if we find it to get the service name for linking -->
                            <template
                              v-for="_hash in [/-([a-f0-9]){16}$/]"
                              :key="typeof _hash"
                            >
                              <XLayout
                                variant="y-stack"
                                size="small"
                              >
                                <template
                                  v-for="[name, outbound] in outbounds"
                                  :key="`${name}`"
                                >
                                  <ConnectionCard
                                    data-testid="dataplane-outbound"
                                    protocol=""
                                    :traffic="outbound"
                                    :direction="direction"
                                  >
                                    <XAction
                                      data-action
                                      :to="{
                                        name: ((name) => name.includes('bound') ? name.replace('-inbound-', '-outbound-') : 'zone-ingress-connection-outbound-summary-stats-view')(String(_route.name)),
                                        params: {
                                          connection: name,
                                        },
                                        query: {
                                          inactive: route.params.inactive,
                                        },
                                      }"
                                    >
                                      {{ name }}
                                    </XAction>
                                  </ConnectionCard>
                                </template>
                              </XLayout>
                            </template>
                          </ConnectionGroup>
                        </DataCollection>
                      </DataCollection>
                    </template>
                  </ConnectionTraffic>
                </XLayout>

                <RouterView
                  v-slot="child"
                >
                  <XDrawer
                    v-if="child.route.name !== route.name"
                    width="670px"
                    @close="function () {
                      route.replace({
                        name: 'zone-ingress-detail-view',
                        params: {
                          proxy: route.params.proxy,
                          proxyType: 'ingresses',
                        },
                        query: {
                          inactive: route.params.inactive ? null : undefined,
                        },
                      })
                    }"
                  >
                    <component
                      :is="child.Component"
                      :data="route.params.subscription.length > 0 ? zoneIngress.zoneIngressInsight.subscriptions : (child.route.name as string).includes('-inbound-') ? [zoneIngress.zoneIngress] : traffic?.outbounds || {}"
                      :networking="zoneIngress.zoneIngress.networking"
                      :overview="props.data"
                    />
                  </XDrawer>
                </RouterView>
              </template>
            </DataLoader>
          </template>
        </DataLoader>
      </XCard>
    </AppView>
  </RouteView>
</template>

<script lang="ts" setup>
import type { ZoneIngressOverview } from '../data'
import StatusBadge from '@/app/common/StatusBadge.vue'
import ConnectionCard from '@/app/connections/components/connection-traffic/ConnectionCard.vue'
import ConnectionGroup from '@/app/connections/components/connection-traffic/ConnectionGroup.vue'
import ConnectionTraffic from '@/app/connections/components/connection-traffic/ConnectionTraffic.vue'
import { sources } from '@/app/connections/sources'
import { useRoute } from '@/app/vue'

const props = defineProps<{
  data: ZoneIngressOverview | Error | undefined
}>()
const _route = useRoute()
</script>
<style lang="scss" scoped>
.service-traffic-card {
  cursor: pointer;
}

.about-subsection {
  border-top: var(--x-border-width-10) solid var(--x-color-border);
  padding-top: var(--x-space-70);
}
</style>
