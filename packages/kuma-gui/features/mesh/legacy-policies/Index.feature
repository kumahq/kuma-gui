Feature: mesh / policies / index
  ...
  Scenario: Navigating from `/dataplanes` to `/policies` redirects
    ...
    And I wait for 100 ms
    And I click the "$policies-tab" element
    Then the URL contains "/meshes/default/policies/mfi"
  ...
  Scenario: Clicking the link goes to the detail page and back again
    ...
    And the "$detail-view" element exists
  ...
  Scenario: Shows legacy policy types if there are any legacy policies applied
    Given the URL "/mesh-insights/default" responds with
      ...
      """
    When I visit the "/meshes/default/policies/mfi" URL
    Then the "[data-testid='policy-type-link-CircuitBreaker']" element exists
    And the "[data-testid='policy-type-link-MeshCircuitBreaker']" element exists
