...
export default {
  ...
  data () {
    return {
      ...
      empty_state: {
        title: 'No Data',
        message: 'There are no Timeouts present.'
      },
      tableData: {
        headers: [
  ...
  methods: {
    ...
    loadData () {
      ...
      const endpoint = () => {
        if (mesh === 'all') {
          return this.$api.getAllTimeouts(params)
        } else if ((query && query.length) && mesh !== 'all') {
          return this.$api.getTimeout(mesh, query, params)
        }

        return this.$api.getAllTimeoutsFromMesh(mesh)
      }

      const getTimeouts = () => endpoint()
        .then(response => {
          const items = () => {
            const r = response
    ...
    getEntity (entity) {
      ...
      if (entity) {
        ...
        return this.$api.getTimeout(entityMesh, entity.name)
          .then(response => {
            if (response) {
