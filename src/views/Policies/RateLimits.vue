...
export default {
  ...
  methods: {
    ...
    loadData () {
      ...
      const endpoint = () => {
        if (mesh === 'all') {
          return this.$api.getAllRateLimits(params)
        } else if ((query && query.length) && mesh !== 'all') {
          return this.$api.getRateLimit(mesh, query, params)
        }

        return this.$api.getAllRateLimitsFromMesh(mesh)
      }

      const getRateLimits = () => endpoint()
        .then(response => {
          const items = () => {
            const r = response
    ...
    getEntity (entity) {
      ...
      if (entity) {
        ...
        return this.$api.getRateLimit(entityMesh, entity.name)
          .then(response => {
            if (response) {
