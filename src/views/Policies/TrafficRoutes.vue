  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        const { data, next } = await getTableData({
          getSingleEntity: this.$api.getTrafficRoute.bind(this.$api),
          getAllEntities: this.$api.getAllTrafficRoutes.bind(this.$api),
          getAllEntitiesFromMesh: this.$api.getAllTrafficRoutesFromMesh.bind(this.$api),
          mesh,
          query,
          size: this.pageSize,
    ...
    getEntity(entity) {
      ...
      if (entity) {
        ...
        return this.$api
          .getTrafficRoute(entityMesh, entity.name)
          .then((response) => {
            if (response) {
              const selected = ['type', 'name', 'mesh']
