    ...
    <FrameSkeleton>
      ...
      >
        <template v-slot:tabHeader>
          <div>
            <h3>Traffic Route:{{ entity.name }}</h3>
          </div>
          <div>
            <EntityURLControl
              :name="entity.name"
    ...
    async loadData(offset = '0') {
      ...
      try {
        const { data, next } = await getTableData({
          getSingleEntity: Kuma.getTrafficRoute.bind(Kuma),
          getAllEntities: Kuma.getAllTrafficRoutes.bind(Kuma),
          getAllEntitiesFromMesh: Kuma.getAllTrafficRoutesFromMesh.bind(Kuma),
          mesh,
          query,
          size: this.pageSize,
          offset,
