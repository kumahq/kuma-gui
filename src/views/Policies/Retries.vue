...
export default {
  ...
  data () {
    return {
      ...
      empty_state: {
        title: 'No Data',
        message: 'There are no Retries present.'
      },
      tableData: {
        headers: [
  ...
  methods: {
    ...
    loadData () {
      ...
      const endpoint = () => {
        if (mesh === 'all') {
          return this.$api.getAllRetries(params)
        } else if ((query && query.length) && mesh !== 'all') {
          return this.$api.getRetry(mesh, query, params)
        }

        return this.$api.getAllRetriesFromMesh(mesh)
      }

      const getRetries = () => endpoint()
        .then(response => {
          const items = () => {
            const r = response
    ...
    getEntity (entity) {
      ...
      if (entity) {
        ...
        return this.$api.getRetry(entityMesh, entity.name)
          .then(response => {
            if (response) {
